...
async function getLinkStoryProfile(){const{data:{user}}=await linkstorySupabase.auth.getUser();if(!user)return{data:null,error:null};return linkstorySupabase.from('profiles').select('id,kx_id,username,display_name,role,bio,avatar_url,recovery_email,preferences,created_at').eq('id',user.id).maybeSingle()}
